...
This is **not** a `*2snipe` integration. It does not sync anything to Snipe-IT itself.
...
- Dry-run safety (applicable to the `run` command)
...
- Snipe-IT API patterns (this tool calls GCP and GitHub APIs, not Snipe-IT directly)
...
| `install <name>` | Download, configure, and optionally schedule an integration |
| `uninstall <name>` | Remove integration, Cloud Run Job, and Scheduler trigger |
| `enable <name>` | Re-enable a paused Cloud Scheduler job |
| `disable <name>` | Pause scheduling without removing the integration |
| `run <name>` | Trigger a Cloud Run Job immediately and tail logs |
| `status` | Show all installed integrations with last-run result and schedule |
| `config <name>` | Re-run the configuration wizard for an installed integration |
...
go.mod              # module: github.com/jackvaughanjr/2snipe-manager
go.sum
...
- Always gate on `snipemgr.yaml` being configured before any GCP calls; detect
  missing config early and prompt a first-time setup flow
...
- The install wizard should detect if `snipemgr.yaml` is not yet configured and
  run a first-time GCP/registry setup flow before the integration install
